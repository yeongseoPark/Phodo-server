...
팀원 : 박영서(BE), 김현태(BE), 정진교(FE), 이호준(FE), 권도희(FE/BE)
...
- 이미지 카테고리 관리 및 검색을 위해 유저가 사진 업로드 시 아래와 같은 단계를 거칩니다.
  1) 대용량 cloud storage에 이미지 원본/썸네일 업로드 및 url 추출
     : 대용량 이미지 관리를 위해 DB에는 url만 저장했으며, 썸네일을 이용해 데이터 볼륨을 감소하고 렌더링 속도를 개선함
  2) vision API와 natural language API를 이용한 카테고리 분류
     : vision API를 이용해 이미지 라벨들을 추출 -> 라벨 관계도를 이용한 DFS 알고리즘으로 라벨 1차 분류 -> 라벨들의 자연어 처리를 통해 2차 분류
     : 1차로 분류한 라벨들의 범위가 너무 넓어서, 자연어 처리를 이용해 라벨들의 관계도를 한 번 더 파악하여 카테고리를 분류함.
  3) EXIF 데이터에서 촬영 시간 및 위치 추출
